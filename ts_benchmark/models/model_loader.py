...
def get_model_info(model_config: Dict) -> Union[Dict, Callable]:
    ...
    model_info = None
    error_records = []  # 核心修改：用列表把每一次尝试的报错及其堆栈都记下来
    ...
    for model_name in model_name_candidates:
        try:
            ...
            # 记录下是哪个路径失败了，以及完整的报错追踪(Traceback)
            error_records.append(f"【尝试路径: {model_name}】失败原因:\n{traceback.format_exc()}")
            continue

    # 如果全军覆没，把所有真实报错一次性打印出来
    if model_info is None and error_records:
        error_details = "\n" + "=" * 70 + "\n"
        error_details += "\n\n".join(error_records)
        error_details += "\n" + "=" * 70
        raise RuntimeError(f"模型加载彻底失败！真实内部报错详情如下:{error_details}")
