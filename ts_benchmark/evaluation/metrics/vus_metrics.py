...
class metricor(_LegacyMetricor):
    """Exact-grid acceleration of the supplied Range-AUC/VUS implementation.

    The same 250 sampled score thresholds (including duplicates), >= ties,
    extension weights, every integer window, endpoints and trapezoids are used.
    Only summation order changes; float64 round-off is expected, not bit identity.
    Non-binary/degenerate/non-finite inputs retain the legacy implementation.
    """

    @staticmethod
    def _fast_inputs(labels, score):
        labels, score = np.asarray(labels), np.asarray(score)
        return (labels.ndim == score.ndim == 1 and labels.size == score.size
                and labels.size > 0 and score.dtype.kind == 'f'
                and np.all(np.isfinite(score))
                and np.all((labels == 0) | (labels == 1))
                and np.any(labels == 0) and np.any(labels == 1))

    def range_convers_new(self, label):
        # Identical inclusive endpoints; also supports the positive soft labels
        # used internally by extension. No Python scan over all N timestamps.
        active = np.asarray(label) != 0
        if active.ndim != 1:
            return super().range_convers_new(label)
        edges = np.flatnonzero(np.diff(np.r_[False, active, False]))
        return [(int(s), int(e - 1)) for s, e in edges.reshape(-1, 2)]

    @staticmethod
    def _threshold_state(score):
        score = np.asarray(score)
        # Deliberately retain the original sampling, not 250 unique quantiles.
        score_sorted = -np.sort(-score)
        thresholds = score_sorted[np.linspace(0, len(score) - 1, 250).astype(int)]
        # bucket[t] is the FIRST sampled threshold accepting score[t].
        # side='left' preserves >= and repeated thresholds exactly.
        bucket = np.searchsorted(-thresholds, -score, side='left')
        counts = np.cumsum(np.bincount(bucket, minlength=250))
        return bucket, counts

    @staticmethod
    def _extend_cached(x, window, ranges):
        # Same operations/order as extend_postive_range, with precomputed ranges.
        label = x.copy().astype(float)
        length = len(label)
        for s, e in ranges:
            x1 = np.arange(e, min(e + window // 2, length))
    ...
    def _curve_from_state(self, labels, P, state):
        bucket, counts = state
        # At threshold j, the selected set is exactly {t: bucket[t] <= j}.
        # Weighted histogram + prefix sum equals sum(labels * (score >= thr)).
        tp = np.cumsum(np.bincount(bucket, weights=labels, minlength=250))
        P_new = (P + np.sum(labels)) / 2
        ranges = self.range_convers_new(labels)
        # Expanded labels are strictly positive within each range. A range has
        # positive overlap iff at least one score passes the current threshold.
        first_hits = [int(np.min(bucket[s:e + 1])) for s, e in ranges]
        existence = np.cumsum(np.bincount(first_hits, minlength=250)) / len(ranges)
        tpr = np.r_[0., np.minimum(tp / P_new, 1.) * existence, 1.]
        fpr = np.r_[0., (counts - tp) / (len(labels) - P_new), 1.]
        prec = np.r_[1., tp / counts]
        # Keep the legacy ROC and PR endpoint/slicing conventions exactly.
        auc = np.sum((fpr[1:] - fpr[:-1]) * ((tpr[1:] + tpr[:-1]) / 2))
        ap = np.sum((tpr[1:-1] - tpr[:-2]) * ((prec[1:] + prec[:-1]) / 2))
        return auc, ap, fpr, tpr, prec

    def RangeAUC(self, labels, score, window=0, percentage=0,
                 plot_ROC=False, AUC_type='window'):
        if not self._fast_inputs(labels, score):
            return super().RangeAUC(labels, score, window, percentage, plot_ROC, AUC_type)
        labels, score = np.asarray(labels), np.asarray(score)
        P = np.sum(labels)
        if AUC_type == 'window':
            expanded = self.extend_postive_range(labels, window=window)
        else:
            expanded = self.extend_postive_range_individual(labels, percentage=percentage)
        result = self._curve_from_state(expanded, P, self._threshold_state(score))
        return result if plot_ROC else result[0]

    def _volume(self, labels_original, score, windowSize, keep_curves):
        labels_original, score = np.asarray(labels_original), np.asarray(score)
        state = self._threshold_state(score)
        ranges = self.range_convers_new(labels_original)
        P = np.sum(labels_original)
        windows = np.arange(0, windowSize + 1, 1)
        tprs, fprs, precs, aucs, aps = [], [], [], [], []
        for window in windows:
            expanded = self._extend_cached(labels_original, window, ranges)
            auc, ap, fpr, tpr, prec = self._curve_from_state(expanded, P, state)
            aucs.append(auc)
            aps.append(ap)
            if keep_curves:
                tprs.append(tpr)
                fprs.append(fpr)
                precs.append(prec)
        # Python sum, not a new integration rule or a sampled window average.
        return (tprs, fprs, precs, windows,
                sum(aucs) / len(windows), sum(aps) / len(windows))

    def RangeAUC_volume(self, labels_original, score, windowSize):
        if not self._fast_inputs(labels_original, score):
            return super().RangeAUC_volume(labels_original, score, windowSize)
        return self._volume(labels_original, score, windowSize, keep_curves=True)

    def RangeAUC_volume_summary(self, labels_original, score, windowSize):
        """Same scalar VUS pair, without allocating unused surface coordinates."""
        if not self._fast_inputs(labels_original, score):
            return super().RangeAUC_volume(labels_original, score, windowSize)[-2:]
        return self._volume(labels_original, score, windowSize, keep_curves=False)[-2:]
