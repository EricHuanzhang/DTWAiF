...
from ts_benchmark.baselines.DTWAiF.utils.tools import solve_pot as _solve_pot_framework
#from ts_benchmark.baselines.DTWAiF.utils.visual import export_academic_topology_figure
#from ts_benchmark.baselines.DTWAiF.utils.visualize_channel_correlation import visualize_channel_correlation
...
class DTWAiF:
    ...
    def detect_label(self, test: pd.DataFrame, test_labels: pd.DataFrame, series_name: str) -> np.ndarray:
        ...
        if getattr(config, 'visualize', False):
            from ts_benchmark.baselines.DTWAiF.utils.visual import export_academic_anomaly_figure
            segments = [
                (15300, 16000)
            ]
            # 1. Genesis选择参数
    ...
    def export_academic_topology_figure(self, data_loader, series_name="Genesis", save_path="./output_images"):
        ...
        with torch.no_grad():
            ...
            # (b) DTWAiF Low-Freq Mask - 突出“晶格化”二值断路特性
            ...
            # (c) DTWAiF Masked Attn - 掩码过滤后的终极高保真物理图谱
