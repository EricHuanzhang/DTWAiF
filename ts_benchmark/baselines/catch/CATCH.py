...
from ts_benchmark.baselines.catch.utils.fre_rec_loss import ArcTanLoss
...
class CATCH:
    def __init__(self, **kwargs):
        ...
        self.criterion_train = ArcTanLoss()  # 有界梯度：为在误差极大时提供更平滑的梯度衰减
    ...
    def detect_fit(self, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for i, (input, target) in enumerate(self.train_data_loader):
                ...
                rec_loss = self.criterion_train(output, input)
            ...
            valid_loss = self.detect_validate(self.valid_data_loader, self.criterion_test)
            print(
